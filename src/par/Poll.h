...
template <typename TResultWait, typename TResultContinue>
struct Poll{
    Poll() = default;
    ...
    Poll(Executor& executor, Calc<TResultWait()> calc_to_wait_for, std::function<TResultContinue(TResultWait)> func_to_continue) : _executor{executor}, _calc_to_wait_for{calc_to_wait_for}, _func_to_continue{func_to_continue} {}
    ...
    void poll() const {
        ...
        auto calc_to_continue = Calc<TResultContinue(TResultWait)>([this](TResultWait resultWait){
            return _func_to_continue(resultWait);
        });
